...
const mapNameFamily = (characters) => {
  ...
  characters.forEach((character) => {
    out.push(`${character.name}, of House ${character.house}`);
  });
  return out;
};
...
const filterFamily = (characters, house) => {
  ...
  let out = [];
  characters.forEach((character) => {
    if (character.house == house) out.push(character);
  });
  return out;
};
...
const reduceHouses = (characters) => {
  ...
  let out = {};
  characters.forEach((character) => {
    if (out[character.house] == undefined) out[character.house] = 1;
    else out[character.house] += 1;
  });
  return out;
};
